    ...
    <header className={`inset-x-0 top-0 z-50 fixed bg-white ${shadow}`}>
        <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end me-7">
            <Link to={cartLink} className="text-sm relative container-icon  font-semibold leading-6 text-gray-900 scale-150">
              <span className="icon absolute -top-3 -right-5 text-gray-500">({Cart.length})</span>
              <FaShopify className="scale-150" />
            </Link>
          </div>
            <Link to={cartLink} className="text-sm lg:hidden absolute right-24 container-icon  font-semibold leading-6 text-gray-900 scale-125">
              <span className="icon absolute -top-3 -right-5 text-gray-500">({Cart.length})</span>
              <FaShopify className="scale-150" />
            </Link>
        </nav>
