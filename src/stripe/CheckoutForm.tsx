...
const CheckoutForm = ({email}:{email:string}) => {
  ...
  return (
   <>
        ...
        <form className='p-3 my-10' id="payment-form" onSubmit={handleSubmit}>
      <LinkAuthenticationElement options={{defaultValues:{email:email}}} id="link-authentication-element"
        />
      <PaymentElement id="payment-element" />
      <button className='btn my-3 p-2 w-full rounded-md font-bold' disabled={isLoading || !stripe || !elements} id="submit">
        <span id="button-text">
          {isLoading ? <svg
                  aria-hidden="true"
                  role="status"
